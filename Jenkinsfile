pipeline {
    ...
    stages{
       ...
       stage("Trivy Scan") {
           steps {
               script {
	               // Clean the Trivy database cache before the scan
                   sh('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy --no-progress clean --all')

                  // Run the Trivy scan after cleaning the cache
